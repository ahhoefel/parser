...
public class LRItem {
  ...
  public LRTable.State toState(Grammar grammar) {
    ...
    for (Map.Entry<Symbol, List<Integer>> entry : shifts.entrySet()) {
      if (entry.getValue().size() > 1) {
        String msg = "Shifts for symbol " + entry.getKey() + ":\n";
        for (Integer stateNum : entry.getValue()) {
          msg += "\t" + stateNum + "\n";
        }
    ...
    for (Symbol terminal : grammar.getTerminals().getSymbols()) {
      ...
      Optional<ShiftReduceResolver.Preference> pref = grammar.getShiftReduceResolver().getPreference(reduceRule,
          terminal);
      if (!pref.isPresent()) {
    ...
    return new LRTable.State(this, reduce, shift, state);
  }
